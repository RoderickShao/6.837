...
int Grid::initializeRayMarch(MarchingInfo &mi, const Ray &r, float tmin) const
{
    ...
    Vec3f p = rayOrigin + mi.tMin * rayDir - pMin;// + Vec3f(0.01,0.01,0.01);
    mi.i = std::min((int) (p.x() / dx), nx - 1);
    mi.j = std::min((int) (p.y() / dy), ny - 1);
    mi.k = std::min((int) (p.z() / dz), nz - 1);

    if (rayDir.x()==0) mi.tNextX = std::numeric_limits<float>::infinity();
    else mi.tNextX  = ((mi.i + (mi.signX < 0 ? 0 : 1)) * dx + pMin.x() - rayOrigin.x()) / rayDir.x();
    if (rayDir.y()==0) mi.tNextY = std::numeric_limits<float>::infinity();
    else mi.tNextY  = ((mi.j + (mi.signY < 0 ? 0 : 1)) * dy + pMin.y() - rayOrigin.y()) / rayDir.y();
    if (rayDir.z()==0) mi.tNextZ = std::numeric_limits<float>::infinity();
    else mi.tNextZ  = ((mi.k + (mi.signZ < 0 ? 0 : 1)) * dz + pMin.z() - rayOrigin.z()) / rayDir.z();
