                 ...
                 float indexOfRefraction, Hit & intersection) const
{
    ...
    if (!group -> intersect(ray, intersection, tmin)) {
    ...
    Vec3f point = intersection.getIntersectionPoint();

    Vec3f color = scene -> getAmbientLight()* intersection.getMaterial() -> getDiffuseColor(point);
    ...
    for (int ilight = 0; ilight < scene -> getNumLights(); ilight++)
    {
        ...
        Hit lightHit;
        if (shadows && group -> intersect(lightray, lightHit, 0) && lightHit.getT() < disToLight) {
        ...
        color += intersection.getMaterial() -> Shade(ray, intersection, dirToLight, lightColor);
    }
    ...
    if (material != nullptr)
    {
        ...
        auto reflectionColor = material -> getreflectiveColor(point);
        if (reflectionColor.Length() > 0)
        {
            ...
            RayTracingStats::IncrementNumNonShadowRays();
            Hit reflectionHit;
            Vec3f reflectColor = traceRay(reflectionRay, 0, bounces + 1, weight * material -> getreflectiveColor(point).Length(), indexOfRefraction, reflectionHit);
        ...
        auto refractionColor = material -> gettransparentColor(point);
        if (refractionColor.Length()>0)
        {
            float index_i = indexOfRefraction;
            float index_t = material -> getindexOfRefraction(point);
            ...
            if(is_refract)
            {
                ...
                RayTracingStats::IncrementNumNonShadowRays();
                Hit refractionHit;
                Vec3f refractColor = traceRay(refractionRay, 0, bounces + 1, weight * material -> gettransparentColor(point).Length(), material -> getindexOfRefraction(point), refractionHit) ;
...
Vec3f RayTracer::traceRayFast(Ray &ray, float tmin, int bounces, float weight, 
                 float indexOfRefraction, Hit & intersection) const
{
    RayTracingStats::IncrementNumNonShadowRays();

    if (!grid -> intersect(ray, intersection, tmin)) {
        return scene -> getBackgroundColor();
    }
    ...
    for (int ilight = 0; ilight < scene -> getNumLights(); ilight++)
    {
        ...
        Ray lightray = Ray(intersection.getIntersectionPoint()-ray.getDirection()*0.001f, dirToLight);
        Hit lightHit;
        if (shadows && grid -> intersect(lightray, lightHit, 0) && lightHit.getT() < disToLight) {
            RayTracingStats::IncrementNumShadowRays();
            RayTree::AddShadowSegment(lightray, 0, lightHit.getT());
            continue;
        }
