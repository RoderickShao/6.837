...
class Grid : public Object3D {
    public:
        Grid(BoundingBox *bb, int nx, int ny, int nz){
            ...
            infinitePrimitives = new Object3DVector;
        }

        ~Grid() {
            delete[] objects;
            delete infinitePrimitives;
        }
        ...
        void insertInfObject(Object3D *obj) {
            infinitePrimitives->addObject(obj);
        }
        ...
        Object3DVector *getObjects(int i, int j, int k) {
            assert(i >= 0 && i < nx);
            assert(j >= 0 && j < ny);
            assert(k >= 0 && k < nz);
            return &objects[i * ny * nz + j * nz + k];
        }

        Object3DVector* getInfinitePrimitives() {
            return infinitePrimitives;
        }
        ...
        */
        ...
        bool isInGrid(Vec3f point, int i, int j, int k) const {
            Vec3f min = boundingBox->getMin();
            float minX = min.x() + i * dx;
            float maxX = minX + dx;
            float minY = min.y() + j * dy;
            float maxY = minY + dy;
            float minZ = min.z() + k * dz;
            float maxZ = minZ + dz;
        
            return (point.x() >= minX-0.03 && point.x() <= maxX+0.03 &&
                    point.y() >= minY-0.03 && point.y() <= maxY+0.03 &&
                    point.z() >= minZ-0.03 && point.z() <= maxZ+0.03);
        }
    ...
    protected:
        ...
        Object3DVector *infinitePrimitives;
};
